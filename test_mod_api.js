#!/usr/bin/env node
/**
 * steveX 验证脚本 —— 采集端 mod WebSocket JSON-RPC API（51 方法）。
...
async function testViaSteveX() {
  ...
  check('方法清单共 51 个', Array.isArray(methods.json.methods) && methods.json.methods.length === 51,
    `count=${methods.json.methods?.length}`)

  // 状态
  const st = await get('/api/mod/status')
  check('GET /api/mod/status 返回 connected + 51 methods',
    typeof st.json.connected === 'boolean' && st.json.methodCount === 51)
  ...
  check('GET /api/skills 返回 51 个方法', skills.json.ok === true && skills.json.skills.length === 51,
