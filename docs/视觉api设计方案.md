...
> - **v2.37（实体减量欠删：占用格含"当前可见方块"⇒ 冻结实体永久残留，§7.14，✅ 已实施·方案 B）**：用户实测"营火弹出的掉落物的消失判定有概率失效"——**不是随机**。根因：§7.11 ⑦ 的实体删除判据是"冻结实体 AABB 覆盖的**全部**格都在 `provenEmpty` 内"，而 `provenEmpty = terrain.deletions() ∪ {cameraCell}`，`deletions` 由采集侧 `DeletionJudge.judge` 产出、其首句就是 `if (currentTerrain.contains(pos)) continue;`（`currentTerrain` = 采集侧本快照**可见方块**）⇒ **采集侧看得见的方块所在格永远进不了 deletions** ⇒ 实体 AABB 只要覆盖任一"当前有可见方块"的格，`allCellsEmpty` 恒 false ⇒ **该实体永久无法删除**。分界恰好是"落点是不是整方块顶"：整方块把实体顶到上方空气格（可证明为空 → 正常删），**非满方块（营火/耕地/雪层/台阶/压力板…）顶面低于整格，实体盒停在方块自身那一格内** → 死锁（实体盒跨格压到相邻可见方块同理）。已用工件级证据钉死（§7.14：镜像 `entities/r.-1.0.mca` 里 `Pos=[-25.01,64.438,19.843]`、`cooked_rabbit ×3`、`Age=-32768` 的冻结物品，AABB floor 后 = `{(-26,64,19),(-25,64,19)}`；`deletions` 有前者、无后者；同会话 4 条 `Removed stale entity` 全在 y=64.0）。**影响面不限掉落物**——任何冻结实体与任一当前可见方块共格即永久幽灵。方案未定：A 豁免可见方块格（需配"至少一票真证据"兜底，否则判据空转、离开视野即删 ⇒ 闪烁）/ B 采集侧直读"该格现实中是否还有实体"并列通道（**推荐**，换判据、免除几何依赖、遮挡/跨界一并归零）/ C 直读客户端追踪集（最精确、工作量最大）/ D 实体盒深度判据（**否决**：实体渲染几何 ≪ AABB ⇒ 误删活体）。详见 §7.14。同步 §7.11 ⑦ 补记。
>   - **裁决与实施（2026-09-12）**：用户选定 **方案 B**，实施设计见 **§7.14.1**，代码已落地两端（cells 升 **version = 6** + 实体段、`terrain.nbt` 并列键 **`entityDeletions`**、采集侧新类 `EntityPresenceCorrector`、记忆侧 ③ 证据集改为 `entityDeletions ∪ {相机格}`）。两端 `compileJava` 通过；游戏内验收见 §7.14.1 末的七条清单（**尚未执行**）。
...
| 问题 | 处理 |
|---|---|
| **命中点在面平面上** | 深度像素的命中点恰在某可见面平面（整数坐标）。**沿射线离相机更远推 ε**：`nudged = world + normalize(world − camPos)·ε`（ε=0.05）再 `floor`——min 面（块[10,11] 的 x=10 → 10.05 → floor 10）与 max 面（草顶 y=65 → 64.95 → floor 64）**均正确**（逐例验证）。<br>⚠️ **v2.39 更正**：旧稿"最薄实心面（压力板 1/16=0.0625）> ε，不穿格"是**模型直觉估计**；实测烘焙几何里最小的"面到本格出口距离"是 **1/64 = 0.015625**（睡莲 / 红石粉），**< ε = 0.05** ⇒ 该面被推出本格、像素归到背面邻格（水/支撑块）⇒ **整类方块永不进 `terrain`**。1/16 类（地毯/藤/雪层）余量只有 **1.25×**。两个方向的约束、实测失效面与仲裁方案见 **§4.2.1** |
| **深度量化**（float32） | 1/z 压缩非线性，误差随距离显著增大：30 格 ≈1.1mm、100 格 ≈1.2cm、**300 格 ≈11cm、768 格 ≈0.7 格**（**v2.11 更正**：原"≤768 格 <1.2cm"仅 ≤~100 格成立）；近中距 ε=0.05 覆盖，远距由**近侧回退**兜底（见 §5.1）；**不查 6 邻居**（防实体邻格假阳性）。<br>**v2.39 补闭式**（由本表同一投影式 `d(z) = A − B/z`、`A = f/(f−n)`、`B = fn/(f−n)` 推出，与上列数值逐项吻合）：`Δz(1 ULP) = 2⁻²⁴·z²/B ≈ 2⁻²⁴·z²/n = 1.1920e-6·z²`（n = 0.05）——**与远平面 f 无关**（B ≈ n），只由近平面定。<br>**两个口径必须分清**：**1 ULP（量化台阶）= 1.192e-6·z²**（本表数值用的是这个）／**单像素误差上界（0.5 ULP）= 5.96e-7·z²**。<br>关键半径（1 ULP 口径）：`= 1/64` → **114.5 格**；`= ε = 0.05` → **204.8 格**（0.5 ULP 口径 289.7 格）；`= 1/16` → 229.0 格；`= 1 格` → 915.9 格 |
...
### 4.2.1 落格 ε：两个约束、实测失效面与仲裁方案（**决策 R**，v2.39，2026-09-11）

ε 只做一件事：把"恰好落在格界平面上的命中点"**确定性地**推入该可见面所属的那一格。它不参与删除判定。

**两个约束作用在不相交的两类面上**——这是"调不出好值"的根因：

| 约束 | 谁要求它 | 数学形式 | 已知实例 |
|---|---|---|---|
| **下界** | **落在格界平面上的面**（此时 `floor` 本身是掷硬币） | `ε > 误差上界 = 0.5·Δz(1ULP) = 5.96e-7·z²` | 原版几乎所有方块的面（都在整数平面上） |
| **上界** | **贴着本格出口的薄几何**（面到本格出口 < ε） | `ε·\|dy\| < 面到本格出口的距离`（= 该方块沿射线的"厚度"） | 睡莲 / 红石粉 = **1/64** |

关键：**落在格内部的面（睡莲上表面在 `y₀+1/64`）对下界毫无要求**——没有格界歧义可解，`floor(W)`
本来就唯一正确，是 ε 把它推出去的。反过来，整数平面上的面对上界毫无要求——那里没有薄几何。
**两个人群不相交，却共用一个常数。**

**实测失效面**（2026-09-11，对当前 `memory_cells.bin` 全部 1585 个形状格逐 quad 扫"面到本格出口距离 `b`"）：

| blockId | 格数 | 水平 quad | `b = min(h, 1−h)` | 逸出条件 | 判定 |
|---|---|---|---|---|---|
| `redstone_wire` | 3 | 28 | 1/64 | 俯角 > 18.2° | ❌ 整块逸出（可见面只有这一层） |
| `iron_bars` | 2 | 32 | ≈0（格底/格顶平面） | 任意角度 | ⚠️ 逸出；按"背面邻格与它同面"**推断**无害，待复核 |
| `seagrass` / `tall_seagrass` / `kelp` / `short_grass` | 746/716/106/12 | 0 | — | — | ✅ 交叉模型无水平面 |

加上用户实测确认的 `lily_pad`（该世界 `terrain.nbt` 中 **0 格**，快照 `agentPitch = 43.8903°` ⇒ `|dy| = 0.693 > 0.3125`）。
**逐像素机制**（1280×720、f = 512、cam = (100.3, 70.0, 200.5)、屏幕中心像素、pitch 43.89°）：

```
d      = 0.99885906                                 ← 该像素深度（A − B/z，z = 40.3705）
W      = (100.3, 42.015625, 229.6033)               ← 睡莲上表面（42 + 1/64），格 (100,42,229)
nudged = W + r̂·0.05 = (100.3, 41.98094, 229.6394)   ← 竖直下落 0.05×0.693 = 0.03466 格
key    = floor(nudged) = (100, 41, 229)             ← 睡莲下方的水格 ✗
```

跨格点在第 `0.015625/0.693 = 0.0225` 格处、**只占 ε 的 45%**。水非空气 ⇒ **§5.1 的 air 近侧回退不触发**
（回退本身是对的，缺的是"背面邻格非空气/非水"这一整类分支）⇒ 该像素被记成水，睡莲格一个像素都拿不到。
**后果不止"看不见"**：`terrain` 同时是删除判定的永不删除集合与记忆世界的内容来源，贴花进不了 `terrain`
⇒ 记忆世界没有它 ⇒ 它被移除时**没有任何判定路径**（2b 需要记忆侧存在该格）。这是**观察侧**缺陷，
与删除侧无关——"红石粉删得掉但看不见"是自洽的。

**ε 的重复定义（4 处独立字面量，均为 0.05）**：

| 常量 | 位置 | 用途 | 应否随本次下调 |
|---|---|---|---|
| `Unprojector.EPSILON` | `Unprojector:39` | **落格推移**（`visibleBlockHits`） | ✅ **只有它需要** |
| `ObjectResolver.EPSILON` | `ObjectResolver:68` | air 回退恢复 `W`（:332）、实体盒外扩（:411）、薄方块三连判 + 前向空扫步长（:599-643） | ❌ 后两项不可降：外扩降到 0.0078 会让 v2.10 修掉的"透过薄实体采到后方方块"复活；空扫步长 0.0078 使迭代数 20 → 128（6.4×） |
| `ObjectResolver.DELTA` | `ObjectResolver:70` | §5.4 另一处深度比较 | ❌ 与落格无关 |
| `DeletionJudge.DELTA_GLOBAL` | `DeletionJudge:52` | 整格判据 δ + `δ_cell` 封顶 | ❌ |

**决策 R（落格仲裁，待审阅）**：

- **R-1（先行，可立即验）**：**先拆常量**（落格 / 回退恢复 / 实体探针 各一份、单一来源；回退必须拿到
  "本像素实际用的推移量"，不得假设等于落格 ε），然后只把**落格** ε 由 `0.05` 下调到
  **`1/128 = 0.0078125`**（1/64 的 2× 余量；1/16 类余量 1.25× → 8×）。
  代价：落格可靠半径 **290 格 → 114 格**（`sqrt(ε / 5.96e-7)`），即 114~290 格区间"落在格界平面上的面"
  开始有部分像素被舍入残差推到邻格而丢失/记错（满块像素多，基本不受影响；薄物受影响）。
- **R-2（终局）**：**不等距推移 → 边界对齐**。把固定 ε 换成"刚够越过边界"的量：

  ```
  ε_need = max(0, 到本格出口方向最近那个格界平面的距离) + k · errBound(z)
  key    = floor(W + r̂ · ε_need)
  ```

  面落在格内部（睡莲 1/64）→ 最近格界在 0.0156 外、误差带只有 0.001 ⇒ `ε_need ≈ 0` ⇒ **不推**，
  `floor(W)` 直接给出睡莲格；面落在整数平面上（沙顶面）→ 距离 0 ⇒ 推 `k·errBound` 出去 ⇒ 仍得沙格，
  且**不再依赖回退的掷硬币**。**接口变化**：`blockHits` 现存 nudged 点、回退靠"减固定 ε"还原 `W`，
  自适应后必须携带本像素实际推移量（或改存 `W` 本身）。
- **R-3（远距，独立课题）**：当 `errBound(z) ≫ 方块厚度`（约 200 格处 `errBound ≈ 0.24 格 ≫ 1/64`），
  深度分辨率本身已无法区分该格，任何推移策略都无解——只能在渲染线程用方块状态仲裁
  （`queryBlocks` 已在渲染线程且持有 `level`，是唯一的仲裁点）。

**验收（游戏内，R-1 / R-2 通用）**：

1. **方向性判据**：钻到水下**向上**看睡莲 → 推移方向朝上、留在格内 ⇒ **下调前就应该能观察到**
   （这条用来证明失明是"推移方向 + 1/64 厚度"的单向效应，不是深度/掩码问题）；
   回到俯角 > 18.2° 俯视 → **下调后**应能观察到（下调前必失败）。
2. **零回归**：地毯 / 雪层 / 藤 / 花 / 草在俯视下仍在 `terrain` 中，且不比下调前变稀。
3. **半径回归**：114~290 格区间的可见集（水下沙地、远处地面）与下调前逐块比对，确认没有成片丢失。
4. **远端既有行为不变**：`Δz(1ULP) > ε` 区（> 204.8 格）本来就在"落格掷硬币"状态，本次不新增该区间的约定。

**实施记录（2026-09-11，R-1a / R-1b / 诊断，`compileJava` 通过）**：

| 落点 | 改动 |
|---|---|
| `Unprojector` | `EPSILON` → **`LANDING_EPSILON = 1/128 = 0.0078125`**（javadoc 记全两个约束、1/64 实测、下界半径代价与 R-2 残留限制） |
| `Unprojector.UnprojectResult` | 新增 `boundaryResolvedPixels`（诊断）与 **`landingEpsilon`**（本次推移的**实际用量**）——后者是 §5.1 回退的必需输入，见下 |
| `ObjectResolver` | `EPSILON` 拆为 **`PROBE_EPSILON`**（实体盒相交探针 / 薄方块三连判 / 扫描长度下限）与 **`SCAN_STEP`**（前向空扫步长，防"全局调小 ε"把这条热循环乘 6.4×）；**air 回退改用 `hits.landingEpsilon()`**，不再读任何常量 |
| `VisionApi` | 显式传 `Unprojector.LANDING_EPSILON`；`[Vision] snapshot:` 行追加 `landingBoundary=N(ε=…)` |

**实施中发现并修掉的一处原计划未列出的隐患（重要）**：`W = nudged − dir·ε` 在数学上是落格推移的
**逆运算**，故回退必须用**落格时实际使用的 ε**。旧代码读的是自己的常量、只是因为两处同值才恰好正确；
若照原计划保留 `ObjectResolver.EPSILON = 0.05` 做回退，还原出的 `W` 会沿射线偏
`0.05 − 1/128 = 0.0422` 格——**大于 1/64 贴花的整个厚度**，`floor(W)` 直接穿到贴花后面那块。
故采纳的接口不是"回退也带个 ε 常量"，而是**由 `UnprojectResult` 携带本次实际推进量**（这也是 R-2
"`blockHits` 必须携带实际推进量"的最小前置步骤）。
...
- **⚠️ 落格量必须与回退量拆开（v2.39 决策 R-1）**：本条存的 nudged 点与 §5.1 回退用的
  `ObjectResolver.EPSILON` 是**两个独立字面量**（各 0.05）。回退按"减固定的 ε"还原 `W`，一旦只改落格量，
  恢复点与落格点就相差 `Δε` ⇒ **静默错位**（无编译错误、无日志、只表现为邻格偶发错记）。
  拆常量时回退必须拿到本像素**实际**推移量；
...
- 记忆侧在 terrain / entity / blockEntity **三通道 sync 之后**应用（增量先于减量，顺序不变）：
  ...
  - 冻结实体：不在当前帧实体快照内、且**全部 AABB 占用格 ∈ deletions** → `EntityRestorer.discard(uuid)`（"本次没看到"≠删，"占用格被证明为空"才删，语义不变）；
    - ⚠️ **2026-09-12 补记（v2.37）：本条对实体有系统性欠删洞**——`deletions` 的首句 `currentTerrain.contains(pos) → continue` 使"当前可见方块所在格"永不入表，而实体 AABB 与非满方块（营火/耕地/雪层/台阶…）共格是常见情形 ⇒ **该实体永久残留**。根因、工件级证据与候选方案见 **§7.14**（**已按方案 B 实施，实施设计见 §7.14.1**——本条 ⑦ 对实体的判据自 v2.37 起整体作废）。
...
> **状态**：v2.36（§7.12）把减量范围扩到流体 ∪ 满格透明，**非满形状方块（写 main：栅栏/铁栏杆/门/台阶/活板门/梯子/火把/压力板…；Fabulous 下写 translucent：玻璃板/红石线…）仍整类欠删**，§7.12 末将其归为"需换成实际渲染形状 AABB 采样、属次级工程"。本节记录经论证的**轻量替代**：不必换采样体积——**保留整格 bbox 判据原样，只对"越票"像素追加一个「射线 ∩ 被记住形状」的几何相交过滤**。方向已与用户讨论定稿并**立项为 v2.37**：正式设计（2026-09-08，**设计待审阅、未实现**）见 `docs/非满形状方块减量设计方案.md`——本文草案保留为摘要，代码另起须先经该设计文档审阅。
...
### 7.14 实体减量：占用格含"当前可见方块"⇒ 冻结实体永久残留（v2.37，2026-09-12 诊断，✅ 已实施·方案 B）

> **状态**：根因已定位并**用工件级证据钉死**（非日志推断）。方案未裁决，待用户定夺后本节补"实施"小节，再动代码。

**现象（用户实测）**：营火被敲掉后弹出的熟兔肉在记忆世界里正常冻结复现；现实中被拾取/消失后，**记忆世界里永久残留**。同一会话里落在普通地面上的同类掉落物却都被正确删除——用户描述为"有概率失效"。**它不是概率，是落点决定。**

**判据现状（§7.11 ⑦ / v2.23）**：记忆侧 `DeletionApplier.apply` 的实体循环——

```java
for (UUID uuid : entities.uuids(dimension)) {
    if (currentEntityUuids.contains(uuid)) continue;             // 仍在"可见实体快照"里 → 不删
    if (entities.allCellsEmpty(dimension, uuid, provenEmpty)) {  // AABB 覆盖的全部格都被证明为空 → 删
        entities.discard(dimension, uuid);
    }
}
```

其中 `provenEmpty = terrain.deletions() ∪ {cameraCell}`；`EntityRestorer.allCellsEmpty` 遍历实体 AABB 覆盖的**每一**格，任一格不在 `provenEmpty` 即 `false`。

**根因（一句话）**：`deletions` 由采集侧 `DeletionJudge.judge` 产出，而该函数的第一条语句是

```java
if (currentTerrain.contains(pos)) continue;   // currentTerrain = terrain.keySet() = 采集侧本快照的可见方块
```

⇒ **采集侧当前看得见的方块，其所在格永远进不了 `deletions`，也就永远进不了 `provenEmpty`**。于是：**实体 AABB 只要覆盖到任一"当前有可见方块"的格，`allCellsEmpty` 恒为 `false` ⇒ 该实体永久无法被删除**（"永久"= 只要那格还可见；那格一直被看到就一直不解除）。

**为何分界在"落点是不是整方块顶"**：整方块的碰撞把实体顶到整数 y，实体盒落在方块**上方**的空气格 ⇒ 那些格可被证明为空 ⇒ 正常删。**非满方块**（营火 / 耕地 / 雪层 / 台阶 / 活板门 / 压力板 / 红石线…）的顶面低于整格，实体盒因此停在**方块自身那一格内** ⇒ 死锁。实体盒跨格压到相邻可见方块（贴墙、跨边界）同理。

**证据（2026-09-12 实机；只读两侧落盘工件 + 镜像存档，未依赖日志推断）**：

1. **现实侧** `terrain.nbt`：`deletions = [(-26,64,19), (-13,67,40)]`；`blocks` 含 `(-25,64,19) → minecraft:campfire`（可见方块，**不在 deletions 里**）。
2. **反向通道** `memory_cells.bin`（v5，记忆侧上报的"当前存在"格）：main 段含 `(-26,64,19)` 与 `(-25,64,19)`；signalLoss 段含 `(-31,63,19)` / `(-25,64,19)` 两条 campfire。营火在 `MemoryCellReporter.computeCells` 的第 ② 段（`isSignalLossBlock`）就 `continue` 短路 ⇒ **它不可能来自方块路径** ⇒ main 段那两条只能来自**冻结实体的 AABB**。
3. **镜像存档** `saves/MemoryWorld/entities/r.-1.0.mca` 区块 (-2,1)**（1.21.11 实体存在独立 `entities/` 文件夹，不在区块 NBT 里）**：一条 `minecraft:item`，`Pos=[-25.01, 64.438, 19.843]`、`Item={count:3, id:minecraft:cooked_rabbit}`、`Motion=0`、`NoGravity=1`、`Invulnerable=1`、`Age=-32768`（**`Age=-32768` 只有本 mod 的 `setUnlimitedLifetime()` 会写出** ⇒ 是本系统冻结的掉落物，且永不自然消失）。
4. **几何对齐**：物品实体盒 = 0.25 见方、`Pos` 为底部中心 ⇒ `x∈[-25.135,-24.885]`、`y∈[64.438,64.688]`、`z∈[19.718,19.968]`；`floor` 后占用格 = **`{(-26,64,19), (-25,64,19)}`** —— 与第 2 条 main 段**逐格一致**。且 `64.438 = 64 + 7/16` 正是**营火顶面高度**（`CampfireBlock` 形状 0.4375）⇒ 物品确实停在营火顶上、**位于营火自己那一格 y=64 内**。
5. **死锁在日志里的直接读数**：`20:17:32 … 2 candidates, 0 visible-skipped, 1 deleted, 1 REFUSED by guard` 与 `discarded 0 entities`——判据**投了**空气侧那一票（`(-26,64,19)` ∈ deletions），而营火格 `(-25,64,19)` 因 `currentTerrain` 跳过**永不入表** ⇒ 差一票 ⇒ 不删。同会话此前 4 条 `Removed stale entity …` **全部 y=64.0**（落在整方块上 → 空气格 → 可证 → 已删）——**同会话内"删得掉/删不掉"的分界正是落点**，这就是"有概率"的全部含义。
6. **会话归属**（确认该物品属本会话被追踪实体，而非跨会话孤儿）：本次会话 `Entity sync` 计数 0 → 2 → 4，共生成 5、丢弃 4，剩 1 —— 与该物品一一对应（4 条丢弃日志都在地面 y=64.0）。

**死亡条件（本次事故的充要形式）**：`∃ c ∈ cells(entity) : realityVisibleBlocks(c)`，其中 `cells(entity)` = 冻结实体 AABB 覆盖的整格集合，`realityVisibleBlocks` = 采集侧本快照 `terrain.blocks` 的格集。等价地说：**实体与某个当前可见方块共格**。

**影响面**：不限掉落物——任何被冻结的实体（生物、盔甲架、物品展示框、船、经验球…）只要与任一当前可见方块共格，就永久残留（幽灵）。这是"该删不删"方向；反过来"被永久遮挡 / 超 `removalMaxRayDist`"的欠删是既定边界（§7.11）不受影响。**修法必须同时守住方向纪律**：不得新引入"误删活体"（§7.11 从头到尾的"宁欠勿过"）。

**候选方案（待裁决）**：

- **A：豁免"可见方块格"（最小改动）**——把判据改为 `∀c ∈ cells : c ∈ provenEmpty ∨ c ∈ realityVisibleBlocks`；集合来源现成：`terrain.nbt.blocks` 记忆侧已在读（`TerrainRestorer.realityBlocks()`，决策 J 的墓碑清理就用它）⇒ **采集侧零改动、无新文件、无升版**。
  - **隐患（必须配兜底）**：判据会**空转**——若实体 AABB 的**全部**格都被豁免（物品完全落在营火格内，是多数情形），"全空"条件恒真 ⇒ 实体一旦离开**可见实体快照**（移出视锥 / 被遮挡 / 背对玩家）就会被删，回头再看又复现 ⇒ **可见闪烁**，且语义从"冻结保留"退化为"离开视野即丢"，与 §7.11 "本次没看到 ≠ 删"直接冲突。**兜底 = 至少有一格必须由 `provenEmpty` 证明**（≥1 票真证据）。加上兜底后 A 只解决"跨格"情形——本次事故里恰好成立（`(-26,64,19)` 是真票），但**物品完全落在营火格内时仍不解决**（最常见的摆放）。
- **B：采集侧直读"该格现实中是否还有实体"（新证据通道）** ⭐ **推荐**——采集侧对记忆侧上报的**实体占用格**在真实世界做一次 AABB 实体查询（`level.getEntities(...)` 覆盖该格并**外扩 1 格保守放大**）：区块已加载 ∧ 查询为空 ⇒ 该格在现实中确无实体 ⇒ 产出 `entityDeletions` **并列通道**（与 §15.2 的 `signalLossDeletions` 同款并列键），记忆侧新增一条实体删除原语（不再走 `allCellsEmpty`）。判据不再依赖任何深度/几何 ⇒ **非满方块 / 遮挡 / 跨格三类一次全解**；且比现判据**更强**：离开视野但活着的实体会被查到 ⇒ 不删，正好消掉现判据"离屏即丢证据"的假阳性来源。
  - **代价/隐患**：新增一条通道（可复用 `terrain.nbt` 顶层并列键，须写明语义；若改 `memory_cells.bin` 段则升 version=6）；实体"刚被移除、移除包在途"的竞态 ⇒ 沿用 §7.1 G2 渲染距离闸门同款作用半径限定 + **≥2 帧去抖**（与 §7.11 的 ≥2 像素阈值同一思路）；被删实体的重建依赖下一次可见快照（与现行为一致）。
- **C：采集侧直读"客户端追踪集"（B 的更强形式）**——采集侧把"客户端当前已知（被服务端追踪）的实体 uuid 集合"落盘（`ClientLevel` 的实体表含**视锥外 / 被遮挡**的实体，严格大于可见快照），记忆侧判据改为：`uuid ∉ 可见快照 ∧ uuid ∉ 追踪集 ∧ 位置在跟踪半径闸门内 ⇒ 删`。
  - **与 B 的差别**：B 查"位置附近还有什么"，C 查"这个 uuid 还在不在"；C 不受"实体移动了"影响（移动中的实体仍在集合里 ⇒ 不删），也不需要外扩盒近似。**最精确**，但需新增 uuid 集合的采集与落盘，且"追踪范围"这条边界（服务端 `clientTrackingRange`）必须显式写清——**工作量最大**。
- **D：实体盒深度判据（不推荐，建议否决）**——对实体 AABB 投影内像素做"越过实体盒"的 §7.11 同款 DepthTest 投票。
  - **否决理由**：实体**渲染几何远小于其 AABB**（细物品、生物模型、盔甲架）⇒ 活体单快照内就有大量"穿过 AABB 空余区"的越票 ⇒ **误删活体**（有害方向，比它要治的幽灵更糟）。要修就得把实体渲染形状搬进通道（等价 §7.13 的几何过滤，但实体无 `getShape()`、须从模型烘焙），成本高于 B/C。

**推荐**：**B**（次选 C，A 仅在"接受完全落在营火格内的物品继续欠删"时可作最小补丁）。理由：① 它是**换判据而非打补丁**——把"实体是否还在"从"格是否可证明为空"这个**对实体的错误代理**换回一次真实世界直查，与 §15 "判据的输入不存在，就换输入"同一条路线；② 免除实体几何依赖，三类死锁同时归零；③ 复用现有并列通道模式，两侧改动量与 §15.2 同阶；④ 唯一新增运行时风险（刚移除、包在途）由距离闸门 + 多帧去抖覆盖，方向保守。

**裁决（2026-09-12，用户）**：**采用方案 B**。次要发现（重启孤儿实体）本版不纳入，另立。

#### 7.14.1 实施设计（方案 B：实体在场直读通道）

**一句话**：实体减量的证据从"**方块格是否被证明为空**"换成"**该格附近现实中还有没有实体**"——由采集侧直读客户端世界给出，与深度场、视锥、图形配置全无关（与 §15 的 `SignalLossCorrector` 同一条路线、同一套纪律）。

**协议改动（两处，各自独立可降级）**：

1. **`memory_cells.bin` 升 version = 6**：末尾追加**实体段**（几何段、信号缺失段之后，本文件最后一段）
   ```
   [..]  int entityCellCount
   [..]  entityCellCount × long（BlockPos.asLong；镜像中<b>冻结实体占用格</b>）
   ```
   与既有各段同款纪律：写方/读方逐字节一致，读方**严格不留尾巴**（`sp != bytes.length → null`），故必须升版（沿用 v4→v5 那条既有约定）。version ≤ 5 旧文件 → 实体段空 ⇒ 该通道静默不删（欠删，方向安全）。
2. **`terrain.nbt` 每维桶顶层并列新键 `entityDeletions`**（`LongArrayTag`，同 `deletions` / `signalLossDeletions` 形态）：采集侧证明"该格现实中已无实体"的格集。记忆侧读不到 → 空 → 只增不删。

**两侧落点**：

| 侧 | 文件 | 改动 |
|---|---|---|
| 记忆 | `MemoryCellReporter` | ① `computeCells` 的冻结实体分支**从 main 段移出**到新的 entity 段；② **逐实体门限**（见下）先筛实体，再逐格距离球过滤；③ 内容指纹必须含实体段（漏掉 → 变了也不重写 → 整条通道静默空转，§15.7 第 8 条同一坑）；④ `writeAtomic` 写实体段 + `VERSION = 6`；⑤ 日志追加 entity 计数 |
| 采集 | **新类 `EntityPresenceCorrector`** | 对实体段逐格做"现实世界直读 + 空判"（见四条硬规则），产出 `entityDeletions` + 逐档记账；与 `SignalLossCorrector` 同构（纯函数、无落盘、诊断归调用方） |
| 采集 | `MemoryCellsReader` | `VERSION_6` + 实体段解析 + `CellsData.entityCells`（旧版 → 空） |
| 采集 | `ObjectResolver` | 与信号缺失通道并列调用（`dimensionOk` 同一道门），打逐档记账 |
| 采集 | `VisionTerrainStore` | 新键 `entityDeletions`（恒写，空 = 本帧无该证据）+ `sync` 参数 + 统计 |
| 记忆 | `TerrainRestorer` | 新键读取 → `TerrainData.entityDeletions`；**必须进内容指纹**（同上） |
| 记忆 | `DeletionApplier` | ③ 冻结实体清理的"已证空格集"由 `deletions ∪ {相机格}` 改为 **`entityDeletions ∪ {相机格}`**；新增一条独立日志 |

**四条硬规则**（每条都是"方向错就误删"的那种，与 §15.4 P1~P4 逐条对应）：

- **P1 读不到 ≠ 不存在**：区块未加载 → `level.getEntities` 返回空，与"真的没有实体"**同形**。故 `level.isLoaded(cell)` 必须是**第一道守卫**，未加载 → 跳过（欠删）。这是本通道唯一的新增前提，也是它唯一的"假空"来源。
- **P2 不得用"本次可见集里没有它"当证据**：本通道的证据是**世界直读**，与"看得见"无关——候选集由记忆侧给定，采集侧**不枚举、不做视锥筛选**。若用"可见实体列表"反推，就等于把"被遮挡的活实体"判成消失（那正是原判据误删方向的来源）。
- **P3 只保留"删除"半个动作**：查到实体 ⇒ 一律不裁决（交正向观测通道）。本通道被授权证明的是"不在"，不是"在"。
- **P4 只在真实世界采集帧成立**：同 §15.4 P4（v2.32 起 `terrain.nbt` 整套语义已建立于此）。

**逐通缉门限（方案的承重设计，压在一个数上）**：`level.isLoaded` 只保证"区块在客户端"，**不保证"服务端在追踪该实体"**——服务端按 `clientTrackingRange`（区块）推送实体，**范围外实体活着也不在客户端表里** ⇒ 若只看 `isLoaded`，界外活体会被判成"已消失"（误删方向）。故门限取

```
gate(type) = min(removalMaxRayDist, max(0, (type.clientTrackingRange() - 1) × 16))
```

- 该值**只有记忆侧能算**（它持有冻结实体的 `EntityType`；`EntityType.clientTrackingRange()` 在 1.21.11 是 public，已 javap 核实）。采集侧因此**不持**"哪些类型追多远"这类知识——与 §15"采集侧刻意不持族名单"同构，避免两份名单漂移。
- `-1` 区块余量：追踪边界按区块量化，与 G2 闸门（`§7.1 G2`，`(rd−2)×16`）同一套"边界内收给余量"的算法，只是余量取 1 区块。
- 外再加 `removalMaxRayDist` 封顶：与方块通道同量级，保证文件不因"玩家类型 32 区块"这类大范围膨胀（**代价**：超出该门限的冻结实体永久欠删——它同时也超出了方块判据的可靠区）。
- 距离度量用**实体 AABB 到 agent 眼睛的最近点距离**（与 G2 的 `nearestDist` 同口径：最近点是全实体各点距离的下界，"最近点都在门内"才是保守的那一侧）。

**为什么替换而不并列**：`allCellsEmpty` 原先吃的证据是 `deletions`（方块格被深度证明为空）。该证据对实体是**错的代理**——实体渲染几何 ≪ 其 AABB（细物品、生物模型），格内空余区的射线会投票"越过"，因此它是原判据里**误删活体**的那条隐蔽通道；而它"该删不删"的一面（§7.14 根因）又已被证明不可修。本版把它整条**撤出实体判定**（实体格不再进 main 段、不再喂深度判据），实体删与不删**只由直读通道决定**。副产物：实体判据从此与 Fabulous / PBO / render distance 全部解耦。

**与既有判据的关系（不变量）**：
- `EntityRestorer.allCellsEmpty` **语义与实现不变**——仍是"该实体 AABB 的全部格都在证据集内"；变的只是**喂给它的证据集**。故"本次没看到 ≠ 删"这条语义、以及"部分格有证据、部分格没有 ⇒ 保留"这条保守性全部原样保留。
- 记忆侧跳过集 `currentEntityUuids`（本帧可见实体）不变。
- 相机格无条件证据不变（v2.22 语义）；`entityDeletions` 与 `signalLossDeletions` **不互相并入**（后者对实体占用零信息量，见 §15.5 与本类 ③ 的说明）。
- `MemoryCellReporter` 的 main 段语义**收窄**为"实心不透明块 + 岩浆"（不再是"+ 冻结实体占用格"），采集侧 `DeletionJudge` 的 main 场判据因此不再接触实体格——文档中所有"+ 冻结实体占用格"的措辞按此更新。

**残余边界（写死，不修）**：
1. 实体超出 `gate(type)` → 不进实体段 → 永久欠删（含"玩家走远"）；
2. 实体所在区块未加载 → 不裁决（欠删）；
3. 现实里实体**移动**到别处（离开原格）→ 原格的镜像副本会被判删——**这不是本版引入的**：旧判据在整方块落点上同样会删（§7.14 证据 6 的 4 条日志即是）；移走的实体下次被看见时由正向通道重建；
4. 同格/邻格恰好站着**别的**实体 → 查到即保留（欠删，宁可少删）；
5. 零体积 AABB（marker 盔甲架）在格边界上：查询盒外扩 `1e-3` 防退化相交判定，**不放宽**到"容忍位移"（位移容差会改变 ③ 的语义）。
6. **（方块侧的窄欠删，需知情）**：实体占用格自 v6 起不再进 main 段 ⇒ 那些"**只**因实体占用而被上报"的格也不再是方块删除候选。受影响者必须同时满足：镜像该格的方块<b>本身不入任何方块段</b>（非满形状 ∧ 有 BE ∧ 不在信号缺失族 ⇒ 今日就属欠删）<b>且</b>该格恰有冻结实体。旧行为下这类格会因实体占用而"顺带"进 main 段、被深度判据删掉方块——**那是偶然覆盖，不是设计**；本版删掉这条偶然路径（方向仍是欠删，不误删）。这类方块的正解是进信号缺失族（§14.4 的 148 载体路线），不是靠实体占格兜底。**实例核查**：本次事故的营火恰在族内（`Blocks.CAMPFIRE`），故营火自身既无覆盖损失、也不需要这条偶然路径。

**验证清单（实施后，游戏内）**：
0. **通道活性先确认（升级配套）**：记忆侧日志应出现 `Wrote … + N entity cells`（N 为冻结实体占用格数，本通道的全部输入），采集侧应出现 `[Vision] entityPresence: candidates=…`。**两端必须配套升级**——旧采集侧（cells version ≤ 5）读到 v6 文件会因版本不符**整份拒绝**（刻意如此，见 §7.14.1 协议段），旧记忆侧则不写实体段；任一情况下实体通道都是"无候选 ⇒ 恒不删"。若日志里 `candidates` 恒为 0 而镜像明明有冻结实体，先查这里。诊断行里 ②`occupied` 是常态档、③`visibleSkipped` 恒应≈0、④`absent` 才是本通道的动作数。
1. **本次事故的复现步骤**：营火旁弹出熟兔肉 → 记忆世界出现该掉落物 → 现实里拾取 → 期望：**≤ 数秒内**记忆世界的掉落物消失（旧版永久残留）；
2. 同场景**对照**：营火上的掉落物**仍在现实里**时，记忆世界的副本不得消失（活体误删回归）；
3. 整方块落点（地面掉落物）回归：仍能删（既有行为不得退化）；
4. **遮挡**：让活实体处于全遮挡位置（墙后）并保持不动，其镜像副本**不得**被删（P2 回归）；
5. 区块未加载：走远到实体所在区块卸载 → 该实体镜像副本**不得**被删（P1 回归）；
...
5. **深度量化**：远距离反投影误差靠 epsilon 沿射线推 + **近侧回退**处理（见 §4.2/§5.1），有极小概率误判相邻方块（float32 精度下 30 格 ≈ 1mm、100 格 ≈ 1.2cm；**v2.11 更正**：>100 格后随 1/z 急剧变粗——300 格 ≈ 11cm、768 格 ≈ 0.7 格，此时 ε 不再覆盖、依赖近侧回退兜底；近中距仍远超方块粒度，实际无需担心）。**v2.39 补闭式与两个口径**：`Δz(1 ULP) = 1.1920e-6·z²`（与远平面无关）、单像素误差上界 = 0.5 ULP = `5.96e-7·z²`；ε = 0.05 的下界覆盖半径 **289.7 格**（0.5 ULP 口径）/ **204.8 格**（1 ULP 口径）。ε 真正的失效面不是这个下界而是**几何上界**——见第 16 条与 §4.2.1。
...
16. **落格 ε 的几何上界：薄贴花不可观测（v2.39，决策 R，详见 §4.2.1）**：ε 对"面到本格出口 < ε"的几何
    会把像素推出本格 ⇒ 该类方块**永不进 `terrain`**；又因 `terrain` 既是记忆世界的内容来源、又是删除判定
    的永不删除集合，其消失也**永不可判**（2b 需要记忆侧存在该格）。实测确认面：**睡莲（该世界 0 格）、
    红石粉（1/64）**，判据为俯角 > 18.2°（`ε·|dy| > 1/64`）；1/16 类（地毯/藤/雪层）余量仅 **1.25×**；
    `iron_bars` 的 `b ≈ 0` 待复核（推断无害）。**这是观察侧缺陷，与删除侧无关**——"红石粉删得掉但看不见"
    是自洽的（2b 不要求该格在 `terrain` 内）。
17. **ε 常量四处重复（v2.39）**：`Unprojector.EPSILON` / `ObjectResolver.EPSILON` / `ObjectResolver.DELTA` /
    `DeletionJudge.DELTA_GLOBAL` 是四个独立的 `0.05` 字面量，语义与量纲各不相同（落格 / 回退+实体探针 /
    §5.4 比较 / δ 封顶）。**任何单点改动都必须先拆常量**，否则回退路径静默错位、或实体盒外扩与空扫步长
    被连带改坏（代价见 §4.2.1 的表）。**只有"落格"那一份需要下调**。
...
### Phase 11：落格 ε 拆常量与下调（v2.39，R-1a/R-1b/诊断 ✅ 已实施 2026-09-11；验收待游戏内）
> 触发：睡莲（`lily_pad`）**观察不到**——该世界 `terrain.nbt` 里 0 格，而快照 `agentPitch = 43.8903°`。
> 根因：ε = 0.05 对 1/64 厚度的贴花会把像素推过本格出口（跨格点只占 ε 的 45%），落进下方的水格；
> 水非空气 ⇒ §5.1 的 air 回退不触发 ⇒ 整类贴花永不进 `terrain`（进而永不可判其消失）。§4.2 原稿
> "最薄实心面 1/16 > ε 不穿格"是模型直觉估计，实测是 **1/64**（第三笔"1/16"账，见 §4.2.1）。
- [x] **R-1a 拆常量（零行为变化，先做）**：`Unprojector.LANDING_EPSILON`（落格）/ `ObjectResolver.
      PROBE_EPSILON`（实体盒探针、薄方块三连判）/ `ObjectResolver.SCAN_STEP`（前向空扫步长）
      **各一份、单一来源**；回退不再读任何常量，改用 `UnprojectResult.landingEpsilon()`
      （**该改动修掉了一处原计划未列出的隐患**：`W = nudged − dir·ε` 是落格推移的**逆运算**，
      两值同值时"读哪个常量"看不出差别，一旦落格量单独下调，拿 0.05 去还原就会沿射线偏
      `0.05 − 1/128 = 0.0422` 格——薄几何体（1/64）上直接穿到它后面那块）
- [x] **R-1b 落格 ε 下调**：`0.05 → 1/128 = 0.0078125`（1/64 的 2× 余量；1/16 类余量 1.25× → 8×；
      下界可靠半径 290 → 114 格，仍覆盖 `removalMaxRayDist = 96`）
- [x] 采集侧诊断：`UnprojectResult` 新增 `boundaryResolvedPixels`（推移**改变了所属格**的像素数，
      即压在格界平面上的面的屏幕占比），随 `[Vision] snapshot: … landingBoundary=N(ε=…)` 打印。
      **它是 R-2 的在线指示器**：ε 仍在量化下界之上时该计数只随场景内容变化；一旦 ε 低于 0.5 ULP，
      这群像素的落格退化成掷硬币、计数会在相邻帧间抖动 ⇒ 该距离上的落格不可信。
      （原计划写的"`ε·|dy| > b` 候选计数"未实现：`b` 是每种形状的几何属性，逐像素求它需要
      §3.2 的烘焙顶点，而那在记忆侧——故改用上述**零依赖**的等价指示器）
- [ ] 游戏内验证（§4.2.1 验收 1~3）：水下向上看睡莲**能**观察到（**下调前即应成立**）；俯角 > 18.2° 时
      睡莲/红石粉进 `terrain`；地毯/雪层/藤/花/草零回归；114~290 格可见集不变稀
- [ ] **R-2（边界对齐）待定案**：改动 `blockHits` 的存点语义 + 新增误差模型常量，须先审阅再实施
- [ ] **R-3（远距仲裁）独立立项**：`errBound(z) ≫ 厚度` 段只能靠渲染线程方块状态仲裁
